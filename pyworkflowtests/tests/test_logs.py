...
from pyworkflow.utils import getLineInFile, LoggingConfigurator, restoreStdoutAndErr
from pyworkflow.utils.process import runJob


def test_SimpleFileLog(testOutputPath):
    # Default generic configuration
    Config.SCIPION_LOG = os.path.join(testOutputPath, "general.log")
    ...
    def testMessage(message, msg_callback, file, shouldExist):
        ...
        assert shouldExist == bool(getLineInFile(message, file))
    ...
    logFn = os.path.join(testOutputPath, 'stdout.log')
    logErrFn = os.path.join(testOutputPath, 'stdErr.log')
